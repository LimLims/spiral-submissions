...
// src/index.js
var CONSENT_VERSION = "v1.0-2025-07-27";
var app = new Hono2();
app.post("/submit", async (c) => {
  const pool = new Mn({ connectionString: c.env.DATABASE_URL });
  try {
    ...
    if (sourceUrl) {
      const { rows } = await pool.query(
        "SELECT 1 FROM submissions WHERE source_url = $1",
        [sourceUrl]
      );
      if (rows.length)
        return c.text("This Source\xA0URL has already been submitted.", 409);
    }
    const submitterIp = c.req.header("CF-Connecting-IP") || c.req.header("X-Forwarded-For") || null;
    await pool.query(
...
// .wrangler/tmp/bundle-2LOlZM/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-2LOlZM/middleware-loader.entry.ts
